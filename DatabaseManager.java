...
public class DatabaseManager {
    ...
    public void crearUsuario(String nombre, String usuario, String contrasena, String tipoUsuario, String llavePublica) throws SQLException {
        String query = "INSERT INTO Usuarios (nombre, usuario, contrasena, tipo_usuario, llave_publica) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setString(1, nombre);
            stmt.setString(2, usuario);
            stmt.setString(3, contrasena); // Debe usarse un hash para la contraseña
    ...
    public void registrarEvaluacion(int idPintura, int idJuez, String calificacion, String comentario, String firmaCiega) throws SQLException {
    ...
    // Obtener evaluaciones de una pintura
    public List<String> obtenerEvaluacionesPorPintura(int idPintura) throws SQLException {
        List<String> evaluaciones = new ArrayList<>();
        String query = "SELECT * FROM Evaluaciones WHERE id_pintura = ?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setInt(1, idPintura);
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    evaluaciones.add("Calificación: " + rs.getString("calificacion") + ", Comentario: " + rs.getString("comentario"));
                }
            }
        }
        return evaluaciones;
